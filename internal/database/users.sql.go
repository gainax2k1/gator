...
const getUserIDBName = `-- name: GetUserIDBName :one
SELECT id
    FROM users
    where name = $1
`

func (q *Queries) GetUserIDBName(ctx context.Context, name string) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, getUserIDBName, name)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
